...
router.post('/register', async (req, res) => {
  ...
  try {
    const existing = await pool.query('SELECT id FROM app_users WHERE username = $1', [username.toLowerCase()]);
    ...
    const { rows } = await pool.query(
      'INSERT INTO app_users (username, password_hash, email) VALUES ($1, $2, $3) RETURNING id, username, email',
...
router.post('/login', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      'SELECT id, username, password_hash, email FROM app_users WHERE username = $1',
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      'SELECT id, email FROM app_users WHERE username = $1',
      [username.toLowerCase()]
    );
...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    const { rows: userRows } = await pool.query(
      'SELECT id FROM app_users WHERE username = $1', [username.toLowerCase()]
    );
...
router.get('/me', authMiddleware, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT id, username, email FROM app_users WHERE id = $1', [req.user.id]
    );
